...
const char * const BITCOIN_CONF_FILENAME = "japancoin.conf";
...
#else
    const char* pszModule = "japancoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JapanCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JapanCoin";
#else
    // Unix
    return pathRet / ".japancoin";
#endif
#endif
}
